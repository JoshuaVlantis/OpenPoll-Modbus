...
## Parity audit — modbustools.com Modbus Poll / Modbus Slave (shipped)

After diffing our feature surface against the bundled `mbpoll.exe` and `mbslave.exe`:

- **Modbus ASCII over Serial** — master + slave (7N1 default per spec)
- **Modbus RTU over UDP** — master + slave (RTU framing inside datagrams)
- **Modbus ASCII over UDP** — master + slave (ASCII framing inside datagrams)
- **FC 08 full sub-function set** — 00 Query echo, 01 Restart Comm, 02 Diag Register, 04 Force Listen, 0A Clear Counters, 0B Bus Msg, 0C Comm Err, 0D Excpt Err, 0E Slave Msg, 0F No Resp, 11 Slave Busy
- **Live chart in OpenSlave** — View → Live Chart…, mirrors OpenPoll's chart
- **XLSX snapshot logger** — Tools → Start XLSX snapshot…, writes real `.xlsx` via ClosedXML
- **Test Center framing preview** — MBAP/RTU/ASCII buttons show wire bytes before sending
- **Export Poll → Slave config** — File → Export active poll: generates `.openslave` workspace
