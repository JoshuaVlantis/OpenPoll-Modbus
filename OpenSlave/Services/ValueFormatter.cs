...
/// <summary>
/// Formats and parses cell values for the slave's register grids.
/// Mirrors OpenPoll's formatter so master and slave produce identical
/// representations for every (type, word-order) pair.
/// </summary>
    ...
    public static string FormatRegister(int[] words, CellDataType type, WordOrder order)
    {
        ...
        switch (type)
        {
            ...
            case CellDataType.Float when words.Length >= 2:
                return BitConverter.Int32BitsToSingle((int)Combine32(words, order))
            ...
            case CellDataType.Double when words.Length >= 4:
                return BitConverter.Int64BitsToDouble((long)Combine64(words, order))
